...
You can reinstall the same or a compatible v3 image later and attach the
...
Action Center removal also deletes local cases, steps, Companion transcripts,
encrypted per-member tokens, and approval history. It does not remove
`tagvico/action` or Tagvico custom-field values already mirrored to Paperless;
remove those in Paperless separately if desired.
