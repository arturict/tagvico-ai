...
declare namespace metadataDiff {
  /** A value that can appear anywhere in a metadata snapshot. */
  type MetadataValue =
    | string
    | number
    | boolean
  ...
  /** A metadata snapshot — the before or after side of a diff. */
  type MetadataSnapshot = Record<string, MetadataValue | undefined>;

  /** One row in the per-field diff produced by {@link compareMetadata}. */
  interface MetadataChange {
    field: string;
    before: MetadataValue | undefined;
    after: MetadataValue | undefined;
    /**
     * Reserved for the future case where the patch partially fails and a
     * per-field error is reported back. Currently always `true` for fields
     * present in `after`.
     */
    applied: boolean;
    error?: string;
...
function compareMetadata(
  before: metadataDiff.MetadataSnapshot = {},
  after: metadataDiff.MetadataSnapshot = {}
): metadataDiff.MetadataChange[] {
  const fields = new Set<string>([
    ...Object.keys(before || {}),
    ...Object.keys(after || {})
  ]);
  const changes: metadataDiff.MetadataChange[] = [];
