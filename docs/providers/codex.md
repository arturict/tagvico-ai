# ChatGPT subscription provider (experimental)
...
After login, Tagvico calls the app-server's official `model/list` method and
renders only the visible models returned for that account. The server-provided
default remains the default when the previously saved model is unavailable.
This avoids hard-coded assumptions about Plus, Pro, Business, rollout, or
...
Select `codex` in Settings afterward. `/api/codex/status` reports account type
and plan without returning credential contents or the account email.
...
- `CODEX_MODEL` starts with a conservative fallback, then the Settings dropdown
  replaces it with the signed-in account's current catalog.
