...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (workspaceFor(user).role !== 'owner') {
      throw new ApiError(403, 'Only the Tagvico owner can approve shared tag changes.');
    }
    const { id } = await params;
    return Response.json(
      tagUnificationService.decide(id, await readJsonBody(request, 8 * 1024), user.username),
